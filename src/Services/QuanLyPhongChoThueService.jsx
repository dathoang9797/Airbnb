...
export const quanLyPhongChoThueService = {
  ...
  capNhatHinhAnhPhongChoThue(idRoom, formData, isLoading =true , isLoadingPopup) {
  ...
  capNhatThongTinPhongChoThue(idRoom, phongChoThue, isLoading =true , isLoadingPopup) {
    const url = `${process.env.REACT_APP_LINK_QUAN_LY_PHONG_CHO_THUE}/${idRoom}`;
    return AxiosClient.put(url, phongChoThue, { headers: { isLoading  , isLoadingPopup } });
  },

  xoaPhongChoThue(idRoom, isLoading =true , isLoadingPopup) {
    const url = `${process.env.REACT_APP_LINK_QUAN_LY_PHONG_CHO_THUE}/${idRoom}`;
